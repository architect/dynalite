...
exports.custom = function (data) {
  if (data.BillingMode == "PAY_PER_REQUEST") {
    ...
    if (data.ProvisionedThroughput.ReadCapacityUnits > 1000000000000)
      return (
        "Given value " +
        data.ProvisionedThroughput.ReadCapacityUnits +
        " for ReadCapacityUnits is out of bounds"
      );
    if (data.ProvisionedThroughput.WriteCapacityUnits > 1000000000000)
      return (
        "Given value " +
        data.ProvisionedThroughput.WriteCapacityUnits +
        " for WriteCapacityUnits is out of bounds"
      );
  }

  var defns = data.AttributeDefinitions.map(function (key) {
    return key.AttributeName;
  });
  var keys = data.KeySchema.map(function (key) {
    return key.AttributeName;
  });
  ...
  if (
    ...
    data.KeySchema.length != data.AttributeDefinitions.length
  )
    return (
  ...
  if (data.LocalSecondaryIndexes) {
    ...
    for (i = 0; i < data.LocalSecondaryIndexes.length; i++) {
      ...
      indexKeys = data.LocalSecondaryIndexes[i].KeySchema.map(function (key) {
        return key.AttributeName;
      }); // eslint-disable-line no-loop-func
      if (
        indexKeys.some(function (key) {
          return !~defns.indexOf(key);
        })
      )
        // eslint-disable-line no-loop-func
      ...
      if (
        data.LocalSecondaryIndexes[i].KeySchema[1] &&
      ...
      if (
        data.LocalSecondaryIndexes[i].Projection.NonKeyAttributes &&
  ...
  if (data.GlobalSecondaryIndexes) {
    ...
    for (i = 0; i < data.GlobalSecondaryIndexes.length; i++) {
      ...
      indexKeys = data.GlobalSecondaryIndexes[i].KeySchema.map(function (key) {
        return key.AttributeName;
      }); // eslint-disable-line no-loop-func
      if (
        indexKeys.some(function (key) {
      ...
      if (
        data.GlobalSecondaryIndexes[i].KeySchema[1] &&
      ...
      if (
        data.GlobalSecondaryIndexes[i].Projection.NonKeyAttributes &&
        projectionType != "INCLUDE"
      )
        return (
          "One or more parameter values were invalid: " +
      ...
      if (
        data.BillingMode == "PAY_PER_REQUEST" &&
        data.GlobalSecondaryIndexes[i].ProvisionedThroughput
      ) {
        return (
          "One or more parameter values were invalid: " +
