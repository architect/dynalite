...
exports.custom = function (data) {
  if (
    ...
    !data.BillingMode &&
    !data.UpdateStreamEnabled &&
    (!data.GlobalSecondaryIndexUpdates ||
      !data.GlobalSecondaryIndexUpdates.length) &&
  ...
  if (data.GlobalSecondaryIndexUpdates) {
    ...
    for (var i = 0; i < length; i++) {
      ...
      var indexName =
        (update.Update && update.Update.IndexName) ||
        (update.Create && update.Create.IndexName) ||
        (update.Delete && update.Delete.IndexName);
      if (indexNames[indexName]) {
        return (
          "One or more parameter values were invalid: " +
          "Only one global secondary index update per index is allowed simultaneously. Index: " +
          indexName
        );
      }
