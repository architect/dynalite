...
function create(options) {
  ...
  if (options.maxItemSizeKb == null)
    options.maxItemSizeKb = exports.MAX_SIZE / 1024;
  options.maxItemSize = options.maxItemSizeKb * 1024;

  var db = levelup(
      options.path ? require("leveldown")(options.path) : memdown()
    ),
    subDbs = Object.create(null),
    tableDb = getSubDb("table");

  // XXX: Is there a better way to get this?
  tableDb.awsAccountId = (
    process.env.AWS_ACCOUNT_ID || "0000-0000-0000"
  ).replace(/[^\d]/g, "");
  tableDb.awsRegion =
    ...
    tableDb.get(name, function (err, table) {
      ...
      if (err) {
        if (err.name == "NotFoundError") {
          ...
          if (!checkStatus)
            err.body.message += ": Table: " + name + " not found";
        }
        return cb(err);
      }
...
function validateItem(dataItem, table) {
  return (
    traverseKey(table, function (attr, type, isHash) {
...
function validateUpdates(attributeUpdates, expressionUpdates, table) {
  ...
  return (
    traverseKey(table, function (attr) {
      var hasKey = false;
    ...
    traverseIndexes(table, function (attr, type, index) {
      ...
      if (expressionUpdates) {
        ...
        actualType =
          attributeUpdates[attr] && attributeUpdates[attr].Value
            ? Object.keys(attributeUpdates[attr].Value)[0]
            : null;
      }
  ...
  return (
      ...
      if (nestedPaths[attr]) {
        return validationError(
          "Key attributes must be scalars; " +
            "list random access '[]' and map lookup '.' are not allowed: Key: " +
            attr
        );
      }
    }) ||
    traverseIndexes(table, function (attr) {
      if (nestedPaths[attr]) {
        return validationError(
          "Key attributes must be scalars; " +
            "list random access '[]' and map lookup '.' are not allowed: IndexKey: " +
            attr
        );
      }
    })
...
function createIndexKey(item, table, keySchema) {
  var tableKeyPieces = [];
  traverseKey(table, function (attr, type) {
    tableKeyPieces.push(item[attr][type], type);
  });
  return (
    createKey(item, table, keySchema) + hashPrefix.apply(this, tableKeyPieces)
  );
}

function traverseKey(table, keySchema, visitKey) {
  if (typeof keySchema == "function") {
    visitKey = keySchema;
...
function toLexiStr(keyPiece, type) {
  ...
  return (
    (bigNum.s == -1 ? "0" : "1") + ("0" + exp.toString(16)).slice(-2) + digits
  );
}
...
function numToBuffer(num) {
  ...
  var scale = num.s,
    mantissa = num.c,
    exponent = num.e + 1,
    appendZero = exponent % 2 ? 1 : 0,
    byteArrayLengthWithoutExponent = Math.floor(
  ...
  if (scale == -1) byteArray[0] ^= 0xffffffff;
...
function itemSize(item, compress, addMetaSize, rangeKey) {
  ...
  return !addMetaSize
    ? size
    : 2 + size + (1 + Math.floor((1 + size) / 3072)) * (18 + rangeKeySize);
}
  ...
  switch (type) {
    ...
    case "N":
      ...
      return (
        ...
        (numDigits % 2 || val.e % 2 ? 0 : 1) +
        (val.s == -1 ? 1 : 0)
      );
    case "SS":
      return val.reduce(function (sum, x) {
        return sum + valSize(x, "S");
      }, 0); // eslint-disable-line no-loop-func
    case "BS":
      return val.reduce(function (sum, x) {
        return sum + valSize(x, "B");
      }, 0); // eslint-disable-line no-loop-func
    case "NS":
      return val.reduce(function (sum, x) {
        return sum + valSize(x, "N");
      }, 0); // eslint-disable-line no-loop-func
    case "NULL":
      return 1;
    case "BOOL":
    ...
    case "L":
      return (
        3 +
        val.reduce(function (sum, val) {
          return sum + 1 + valSizeWithStorage(val, compress);
        }, 0)
      );
    case "M":
...
function addConsumedCapacity(data, isRead, newItem, oldItem) {
  if (~["TOTAL", "INDEXES"].indexOf(data.ReturnConsumedCapacity)) {
    ...
    if (oldItem != null) {
      capacity = Math.max(
        capacity,
        capacityUnits(oldItem, isRead, data.ConsistentRead)
      );
    }
    return {
...
function valsEqual(val1, val2) {
  if (Array.isArray(val1) && Array.isArray(val2)) {
    ...
    return val1.every(function (val) {
      return ~val2.indexOf(val);
    });
  } else {
    return val1 == val2;
  }
...
function matchesExprFilter(item, expr) {
  if (expr.type == "and") {
    return (
      matchesExprFilter(item, expr.args[0]) &&
      matchesExprFilter(item, expr.args[1])
    );
  ...
  var args = expr.args.map(function (arg) {
    return resolveArg(arg, item);
  });
  return compare(
    expr.type == "function" ? expr.name : expr.type,
    args[0],
    args.slice(1)
  );
}
...
function resolveArg(arg, item) {
  if (Array.isArray(arg)) {
    ...
    var args = arg.args.map(function (arg) {
      return resolveArg(arg, item);
    });
    var val = args[0],
      length;
  ...
  switch (comp) {
    ...
    case "<=":
      if (
        ...
        (attrType != "N" &&
          toLexiStr(attrVal, attrType) > toLexiStr(compVal, attrType))
      )
        return false;
      break;
    case "LT":
    ...
    case "in":
      if (
        !compVals.some(function (compVal) {
    ...
    case "between":
      if (
        ...
        compType != attrType ||
        (attrType == "N" &&
          (!new Big(attrVal).gte(compVal) ||
            !new Big(attrVal).lte(compVals[1].N))) ||
        (attrType != "N" &&
          (toLexiStr(attrVal, attrType) < toLexiStr(compVal, attrType) ||
            toLexiStr(attrVal, attrType) >
              toLexiStr(compVals[1][compType], attrType)))
      )
        return false;
      break;
    case "attribute_type":
...
function contains(compType, compVal, attrType, attrVal) {
  ...
  if (compType === "B") {
    ...
    return attrVal.some(function (val) {
      ...
      if (attrType === "L" && val.B)
        return compValString === Buffer.from(val.B, "base64").toString();
      return false;
    });
  }
}
...
function mapPaths(paths, item) {
  var returnItem = Object.create(null),
    toSquash = [];
...
function queryTable(
  ...
  var tableCapacity = 0,
    indexCapacity = 0,
    calculateCapacity = ~["TOTAL", "INDEXES"].indexOf(
  ...
  var size = 0,
    count = 0,
    rangeKey = table.KeySchema[1] && table.KeySchema[1].AttributeName;
  ...
  vals.join(function (items) {
    ...
    if (data._filter) {
      items = items.filter(function (val) {
        return matchesExprFilter(val, data._filter.expression);
      });
    } else if (queryFilter) {
      items = items.filter(function (val) {
        return matchesFilter(val, queryFilter, data.ConditionalOperator);
      });
    }
    ...
    var paths = data._projection
      ? data._projection.paths
    ...
    if (calculateCapacity) {
      ...
      if (data.ReturnConsumedCapacity == "INDEXES") {
        ...
        if (data.IndexName) {
          var indexAttr = isLocal
            ? "LocalSecondaryIndexes"
            : "GlobalSecondaryIndexes";
          result.ConsumedCapacity[indexAttr] = {};
          result.ConsumedCapacity[indexAttr][data.IndexName] = {
            CapacityUnits: indexUnits,
          };
        }
      }
    }
...
function updateIndexes(store, table, existingItem, item, cb) {
  ...
  ["Local", "Global"].forEach(function (indexType) {
    ...
    puts = puts.concat(
      actions.puts.map(function (action) {
        var indexDb = store.getIndexDb(
          indexType,
          table.TableName,
          action.index
    ...
    deletes = deletes.concat(
      actions.deletes.map(function (action) {
        var indexDb = store.getIndexDb(
          indexType,
          table.TableName,
          action.index
...
function getIndexActions(indexes, existingItem, item, table) {
  var puts = [],
    deletes = [],
    tableKeys = table.KeySchema.map(function (key) {
  ...
  indexes.forEach(function (index) {
    ...
    if (
      item &&
      indexKeys.every(function (key) {
        return item[key] != null;
      })
    ) {
      if (index.Projection.ProjectionType != "ALL") {
        var indexAttrs = indexKeys.concat(
          tableKeys,
