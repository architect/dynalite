...
describe("listTagsOfResource", function () {
  ...
  describe("validations", function () {
    ...
    it("should return ResourceNotFoundException if ResourceArn does not exist", function (done) {
      ...
      assertNotFound(
        { ResourceArn: resourceArn },
        "Requested resource not found: ResourcArn: " +
          resourceArn +
          " not found",
        done
      );
  ...
  describe("functionality", function () {
    ...
    it("should succeed if valid resource and has multiple tags", function (done) {
      ...
      request(opts({ ResourceArn: resourceArn }), function (err, res) {
        ...
        request(
          helpers.opts("TagResource", { ResourceArn: resourceArn, Tags: tags }),
          function (err, res) {
            if (err) return done(err);
            res.statusCode.should.equal(200);
            ...
            request(opts({ ResourceArn: resourceArn }), function (err, res) {
              ...
              res.body.Tags.forEach(function (tag) {
                tags.should.containEql(tag);
              });

              var tagKeys = tags.map(function (tag) {
                return tag.Key;
              });
