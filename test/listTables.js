...
describe("listTables", function () {
  describe("serializations", function () {
    it("should return 400 if no body", function (done) {
      request(
        { headers: { "x-amz-target": helpers.version + "." + target } },
        function (err, res) {
  ...
  describe("validations", function () {
    ...
    it("should return ValidationExceptions for short ExclusiveStartTableName", function (done) {
      assertValidation(
        { ExclusiveStartTableName: "a;", Limit: 500 },
        [
    ...
    it("should return ValidationException for long ExclusiveStartTableName", function (done) {
      ...
      assertValidation(
        ...
        "1 validation error detected: " +
          "Value '" +
          name +
          "' at 'exclusiveStartTableName' failed to satisfy constraint: " +
  ...
  describe("functionality", function () {
    ...
    it("should return 200 if null attributes are supplied", function (done) {
      request(
        opts({ ExclusiveStartTableName: null, Limit: null }),
        function (err, res) {
          if (err) return done(err);
          res.statusCode.should.equal(200);
          res.body.TableNames.should.be.an.instanceOf(Array);
    ...
    it("should return 200 if correct types are supplied", function (done) {
      request(
        opts({ ExclusiveStartTableName: "aaa", Limit: 100 }),
        function (err, res) {
          if (err) return done(err);
          res.statusCode.should.equal(200);
          res.body.TableNames.should.be.an.instanceOf(Array);
    ...
    it("should return 200 if using query string signing", function (done) {
      ...
      request(requestOpts, function (err, res) {
        ...
        res.statusCode.should.equal(200);
        res.body.TableNames.should.be.an.instanceOf(Array);
        Object.keys(requestOpts.headers)
          .sort()
          .should.eql(["Content-Type", "Host", "X-Amz-Target"]);
        done();
      });
    });
        ...
        function (err) {
          ...
          async.parallel(
            [
              function (done) {
                request(
                  opts({ ExclusiveStartTableName: names[0] }),
                  function (err, res) {
                    if (err) return done(err);
                    res.statusCode.should.equal(200);
              ...
              function (done) {
                request(
                  opts({ ExclusiveStartTableName: beforeName }),
                  function (err, res) {
                    if (err) return done(err);
                    res.statusCode.should.equal(200);
              ...
              function (done) {
                request(
                  opts({ ExclusiveStartTableName: beforeName, Limit: 1 }),
                  function (err, res) {
                    if (err) return done(err);
                    res.statusCode.should.equal(200);
    ...
    it("should have no LastEvaluatedTableName if the limit is large enough", function (done) {
      request(opts({ Limit: 100 }), function (err, res) {
        ...
        request(
          opts({ Limit: res.body.TableNames.length }),
          function (err, res) {
            if (err) return done(err);
            res.statusCode.should.equal(200);
            should.not.exist(res.body.LastEvaluatedTableName);
