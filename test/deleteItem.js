  ...
  describe("validations", function () {
    ...
    it("should return ValidationException if ComparisonOperator used alone", function (done) {
      assertValidation(
        ...
        "One or more parameter values were invalid: Value or AttributeValueList must be used with ComparisonOperator: LT for Attribute: a",
        done
      );
    });
      ...
      assertValidation(
        {
          TableName: "aaa",
          Key: {},
          Expected: { a: { Exists: true, ComparisonOperator: "LT" } },
        },
        "One or more parameter values were invalid: Exists and ComparisonOperator cannot be used together for Attribute: a",
        done
      );
    });
      ...
      assertValidation(
        {
          TableName: "aaa",
          Key: {},
          Expected: { a: { AttributeValueList: [] } },
        },
        "One or more parameter values were invalid: AttributeValueList can only be used with a ComparisonOperator for Attribute: a",
        done
      );
    });
      ...
      assertValidation(
        {
          TableName: "aaa",
          Key: {},
          Expected: { a: { Exists: true, AttributeValueList: [] } },
        },
        "One or more parameter values were invalid: AttributeValueList can only be used with a ComparisonOperator for Attribute: a",
        done
      );
      ...
      assertValidation(
        {
          TableName: "aaa",
          Key: {},
          Expected: { a: { Value: { S: "a" }, AttributeValueList: [] } },
        },
        "One or more parameter values were invalid: Value and AttributeValueList cannot be used together for Attribute: a",
        done
      );
    });
    ...
    it("should return ValidationException if Value provides incorrect number of attributes: BETWEEN", function (done) {
      ...
      assertValidation(
        { TableName: "aaa", Key: {}, Expected: expected },
    ...
    it("should return ValidationException if Value provides incorrect number of attributes: NULL", function (done) {
      ...
      assertValidation(
        { TableName: "aaa", Key: {}, Expected: expected },
        "One or more parameter values were invalid: Invalid number of argument(s) for the NULL ComparisonOperator",
        done
      );
    });
    ...
    it("should return ValidationException if ConditionExpression BETWEEN args have different types", function (done) {
      assertValidation(
        ...
        "Invalid ConditionExpression: The BETWEEN operator requires same data type for lower and upper bounds; " +
          "lower bound operand: AttributeValue: {N:1}, upper bound operand: AttributeValue: {S:a}",
        done
      );
    });
    ...
    it("should return ValidationException if ConditionExpression BETWEEN args are in the incorrect order", function (done) {
      assertValidation(
        ...
        "Invalid ConditionExpression: The BETWEEN operator requires upper bound to be greater than or equal to lower bound; " +
          "lower bound operand: AttributeValue: {S:b}, upper bound operand: AttributeValue: {S:a}",
        done
      );
    });
  ...
  describe("functionality", function () {
    ...
    it("should delete item successfully", function (done) {
      ...
      request(
        ...
        function (err, res) {
          ...
          res.statusCode.should.equal(200);
          request(
            opts({ TableName: helpers.testHashTable, Key: { a: item.a } }),
            function (err, res) {
              if (err) return done(err);
              res.statusCode.should.equal(200);
              res.body.should.eql({});
    ...
    it("should succeed if conditional key is same and exists is true", function (done) {
      async.forEach(
        ...
        function (deleteOpts, cb) {
          var item = {
            a: deleteOpts.Expected
              ? deleteOpts.Expected.a.Value
              : deleteOpts.ExpressionAttributeValues[":a"],
          };
          request(
            helpers.opts("PutItem", {
              TableName: helpers.testHashTable,
    ...
    it("should succeed for multiple conditional checks if all are valid", function (done) {
      async.forEach(
        [
          {
            Expected: {
              a: { Value: { S: helpers.randomString() } },
              b: { Exists: false },
              c: { Value: { S: helpers.randomString() } },
            },
          },
          {
            ConditionExpression:
              "a = :a AND attribute_not_exists(b) AND c = :c",
            ExpressionAttributeValues: {
              ":a": { S: helpers.randomString() },
        ...
        function (deleteOpts, cb) {
          var item = deleteOpts.Expected
            ? { a: deleteOpts.Expected.a.Value, c: deleteOpts.Expected.c.Value }
            : {
                a: deleteOpts.ExpressionAttributeValues[":a"],
                c: deleteOpts.ExpressionAttributeValues[":c"],
              };
          request(
            helpers.opts("PutItem", {
              TableName: helpers.testHashTable,
