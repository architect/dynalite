...
describe("getItem", function () {
  ...
  describe("validations", function () {
    ...
    it("should return ValidationException duplicate values in AttributesToGet", function (done) {
      assertValidation(
        { TableName: "abc", Key: {}, AttributesToGet: ["a", "a"] },
      ...
      async.forEach(
        ...
        function (expr, cb) {
          assertValidation(
            ...
            "Invalid ProjectionExpression: An expression attribute name used in the document path is not defined; attribute name: #a",
            cb
          );
        },
        done
    ...
    it("should return ValidationException if string key has empty string", function (done) {
      assertValidation(
        { TableName: helpers.testHashTable, Key: { a: { S: "" } } },
        "One or more parameter values were invalid: " +
          "The AttributeValue for a key attribute cannot contain an empty string value. Key: a",
        done
      );
    });
    ...
    it("should return ValidationException if binary key has empty string", function (done) {
      assertValidation(
        {
          TableName: helpers.testRangeBTable,
          Key: { a: { S: "a" }, b: { B: "" } },
        },
        "One or more parameter values were invalid: " +
          "The AttributeValue for a key attribute cannot contain an empty binary value. Key: b",
        done
      );
    });

    it("should return ValidationException if hash key is too big", function (done) {
      var keyStr = (helpers.randomString() + new Array(2048).join("a")).slice(
        0,
        2049
      );
      assertValidation(
        { TableName: helpers.testHashTable, Key: { a: { S: keyStr } } },
        "One or more parameter values were invalid: " +
    ...
    it("should return ValidationException if range key is too big", function (done) {
      var keyStr = (helpers.randomString() + new Array(1024).join("a")).slice(
        0,
        1025
      );
      assertValidation(
      ...
      async.forEach(
        ...
        function (expr, cb) {
          assertValidation(
            ...
            "Key attributes must be scalars; list random access '[]' and map lookup '.' are not allowed: IndexKey: d",
            cb
          );
        },
        done
  ...
  describe("functionality", function () {
    ...
    before(function (done) {
      ...
      async.forEach(
        putItems,
        function (putItem, cb) {
          request(helpers.opts("PutItem", putItem), cb);
        },
        done
      );
    });
    ...
    it("should only return requested attributes", function (done) {
      async.forEach(
        [
          ...
          {
            ProjectionExpression: "#b, #g",
            ExpressionAttributeNames: { "#b": "b", "#g": "g" },
          },
        ],
        function (getOpts, cb) {
        ...
        function (err, res) {
          ...
          async.forEach(
            [
              ...
              {
                ProjectionExpression: "#b.#c,#c[2],#b.#b,#c[1],#c[0][1]",
            ...
            function (getOpts, cb) {
              ...
              request(opts(getOpts), function (err, res) {
                ...
                res.body.should.eql({
                  Item: {
                    b: { M: { b: item.b.M.b, c: item.b.M.c } },
                    c: { L: [item.c.L[1], item.c.L[2]] },
                  },
                });
                cb();
              });
    ...
    it("should return ConsumedCapacity for small item with ConsistentRead", function (done) {
      ...
      var items = [
        ...
        },
        {
          ...
          abcdef: {
            M: { aa: { S: "aa" }, bbb: { N: "12.3456" }, cccc: { B: "AQI=" } },
          },
        },
      ];
