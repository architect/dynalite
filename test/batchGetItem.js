...
describe("batchGetItem", function () {
  describe("serializations", function () {
    ...
    it("should return SerializationException when RequestItems.Attr.ExpressionAttributeNames is not a map", function (done) {
      assertType(
        "RequestItems.Attr.ExpressionAttributeNames",
    ...
    it("should return SerializationException when RequestItems.Attr.ExpressionAttributeNames.Attr is not a string", function (done) {
      assertType(
        "RequestItems.Attr.ExpressionAttributeNames.Attr",
        "String",
        done
      );
    });
  ...
  describe("validations", function () {
    ...
    it("should return ValidationException for incorrect attributes", function (done) {
      assertValidation(
        { RequestItems: { "aa;": {} }, ReturnConsumedCapacity: "hi" },
        [
          "Value 'hi' at 'returnConsumedCapacity' failed to satisfy constraint: " +
            "Member must satisfy enum value set: [INDEXES, TOTAL, NONE]",
          new RegExp(
    ...
    it("should return ValidationException for invalid values in Key", function (done) {
      async.forEach(
        ...
        function (expr, cb) {
          assertValidation(
            { RequestItems: { abc: { Keys: [{ a: expr[0] }] } } },
    ...
    it("should return ValidationException for multiple datatypes in Key", function (done) {
      assertValidation(
        { RequestItems: { abc: { Keys: [{ a: { S: "a", N: "1" } }] } } },
    ...
    it("should return ValidationException duplicate values in AttributesToGet", function (done) {
      assertValidation(
        { RequestItems: { abc: { Keys: [{}], AttributesToGet: ["a", "a"] } } },
        "One or more parameter values were invalid: Duplicate value in attribute name: a",
        done
      );
    });
  ...
  describe("functionality", function () {
    ...
    it("should return ConsumedCapacity from each specified table with no consistent read and small item", function (done) {
      var a = helpers.randomString(),
        b = new Array(4082 - a.length).join("b"),
        item = {
          a: { S: a },
          b: { S: b },
          c: { N: "12.3456" },
    ...
    it("should return ConsumedCapacity from each specified table with no consistent read and larger item", function (done) {
      var a = helpers.randomString(),
        b = new Array(4084 - a.length).join("b"),
        item = {
          a: { S: a },
          b: { S: b },
          c: { N: "12.3456" },
    ...
    it.skip("should return all items if just under limit", function (done) {
      ...
      var i,
        item,
        items = [],
        b = new Array(helpers.MAX_SIZE - 6).join("b"),
        batchReq = { RequestItems: {}, ReturnConsumedCapacity: "TOTAL" };
      for (i = 0; i < 4; i++) {
        if (i < 3) {
          ...
          function (err) {
            ...
            request(opts(batchReq), function (err, res) {
              ...
              res.body.ConsumedCapacity.should.eql([
                { CapacityUnits: 357, TableName: helpers.testHashTable },
              ]);
              res.body.UnprocessedKeys.should.eql({});
      ...
      helpers.batchWriteUntilDone(
        ...
        function (err) {
          ...
          request(opts(batchReq), function (err, res) {
            if (err) return done(err);
            res.statusCode.should.equal(200);
            res.body.UnprocessedKeys[
              helpers.testHashTable
            ].ConsistentRead.should.equal(true);
            res.body.UnprocessedKeys[
              helpers.testHashTable
            ].Keys.should.have.length(1);
            Object.keys(
              res.body.UnprocessedKeys[helpers.testHashTable].Keys[0]
            ).should.have.length(1);
            if (
              res.body.UnprocessedKeys[helpers.testHashTable].Keys[0].a.S ==
              "03"
            ) {
              res.body.ConsumedCapacity.should.eql([
                { CapacityUnits: 301, TableName: helpers.testHashTable },
              ]);
            } else {
              res.body.UnprocessedKeys[
                helpers.testHashTable
              ].Keys[0].a.S.should.be.above(-1);
              res.body.UnprocessedKeys[
                helpers.testHashTable
              ].Keys[0].a.S.should.be.below(4);
              res.body.ConsumedCapacity.should.eql([
                { CapacityUnits: 258, TableName: helpers.testHashTable },
              ]);
            }
            res.body.Responses[helpers.testHashTable].should.have.length(3);
            helpers.clearTable(helpers.testHashTable, "a", done);
    ...
    it.skip("should return many unprocessed items if very over the limit", function (done) {
      ...
      var i,
        item,
        items = [],
        b = new Array(helpers.MAX_SIZE - 3).join("b"),
        batchReq = { RequestItems: {}, ReturnConsumedCapacity: "TOTAL" };
      for (i = 0; i < 20; i++) {
        if (i < 3) {
          item = { a: { S: ("0" + i).slice(-2) }, b: { S: b } };
      ...
      helpers.batchBulkPut(helpers.testHashTable, items, function (err) {
        ...
        request(opts(batchReq), function (err, res) {
          ...
          totalLength =
            res.body.Responses[helpers.testHashTable].length +
          ...
          for (
            i = 0;
            i < res.body.UnprocessedKeys[helpers.testHashTable].Keys.length;
            i++
          )
            totalCapacity +=
              res.body.UnprocessedKeys[helpers.testHashTable].Keys[i].a.S < 3
                ? 99
                : 4;
