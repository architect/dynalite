...
exports.testHashNTable = process.env.REMOTE
  ? "__dynalite_test_2"
  : randomName();
exports.testRangeTable = process.env.REMOTE
  ? "__dynalite_test_3"
  : randomName();
exports.testRangeNTable = process.env.REMOTE
  ? "__dynalite_test_4"
  : randomName();
exports.testRangeBTable = process.env.REMOTE
  ? "__dynalite_test_5"
  : randomName();
...
var port = 10000 + Math.round(Math.random() * 10000),
  requestOpts = process.env.REMOTE
    ? {
        host: "dynamodb." + exports.awsRegion + ".amazonaws.com",
        method: "POST",
      }
...
function request(opts, cb) {
  ...
  for (var key in requestOpts) {
    if (opts[key] === undefined) opts[key] = requestOpts[key];
  }
  if (!opts.noSign) {
    aws4.sign(opts);
  ...
  http
    ...
    .on("error", function (err) {
      if (
        err &&
        ~["ECONNRESET", "EMFILE", "ENOTFOUND"].indexOf(err.code) &&
        opts.retries <= MAX_RETRIES
      ) {
        opts.retries++;
...
function createTestTables(done) {
  ...
  var readCapacity = exports.readCapacity,
    writeCapacity = exports.writeCapacity;
  var tables = [
    {
...
function getAccountId(done) {
  request(
    opts("DescribeTable", { TableName: exports.testHashTable }),
    function (err, res) {
      if (err) return done(err);
...
function waitUntilActive(name, done) {
  request(opts("DescribeTable", { TableName: name }), function (err, res) {
    ...
    if (res.statusCode != 200)
      return done(new Error(res.statusCode + ": " + JSON.stringify(res.body)));
    if (
      res.body.Table.TableStatus == "ACTIVE" &&
      (!res.body.Table.GlobalSecondaryIndexes ||
        res.body.Table.GlobalSecondaryIndexes.every(function (index) {
    ...
    request(
      ...
      function (err, res) {
        ...
        if (/ProvisionedThroughputExceededException/.test(res.body.__type)) {
          ...
          return cb(
            new Error(res.statusCode + ": " + JSON.stringify(res.body))
          );
        }
        ...
        for (batchDeletes = []; keys.length; keys = keys.slice(25))
          batchDeletes.push(
            batchWriteUntilDone.bind(null, name, { deletes: keys.slice(0, 25) })
          );
...
function batchBulkPut(name, items, segments, done) {
  ...
  async.eachLimit(
    ...
    segments,
    function (items, cb) {
      batchWriteUntilDone(name, { puts: items }, cb);
    },
    done
  );
}
...
function batchWriteUntilDone(name, actions, cb) {
  var batchReq = { RequestItems: {} },
    batchRes = {};
  batchReq.RequestItems[name] = (actions.puts || [])
    .map(function (item) {
      return { PutRequest: { Item: item } };
    })
    .concat(
      (actions.deletes || []).map(function (key) {
  ...
  async.doWhilst(
    function (cb) {
      request(opts("BatchWriteItem", batchReq), function (err, res) {
        ...
        batchRes = res;
        if (
          res.body.UnprocessedItems &&
          Object.keys(res.body.UnprocessedItems).length
        ) {
          batchReq.RequestItems = res.body.UnprocessedItems;
        } else if (
          /ProvisionedThroughputExceededException/.test(res.body.__type)
        ) {
          console.log("ProvisionedThroughputExceededException"); // eslint-disable-line no-console
    ...
    function () {
      return (
        (batchRes.body.UnprocessedItems &&
          Object.keys(batchRes.body.UnprocessedItems).length) ||
  ...
  switch (type) {
    ...
    case "Blob":
      msgs = [
        ...
        [
          "23456",
          "Base64 encoded length is expected a multiple of 4 bytes but found: 5",
        ],
...
function strDecrement(str, regex, length) {
  ...
  var lastIx = str.length - 1,
    lastChar = str.charCodeAt(lastIx) - 1,
    prefix = str.slice(0, lastIx),
    finalChar = 255;
  while (lastChar >= 0 && !regex.test(String.fromCharCode(lastChar)))
    lastChar--;
  if (lastChar < 0) return prefix;
  prefix += String.fromCharCode(lastChar);
  while (finalChar >= 0 && !regex.test(String.fromCharCode(finalChar)))
    finalChar--;
  if (lastChar < 0) return prefix;
