...
describe("updateItem", function () {
  describe("serializations", function () {
    ...
    it("should return SerializationException when AttributeUpdates.Attr is not a struct", function (done) {
      assertType(
        ...
        "ValueStruct<AttributeValueUpdate>",
        done
      );
    });
    ...
    it("should return SerializationException when AttributeUpdates.Attr.Value is not an attr struct", function (done) {
      this.timeout(60000);
      assertType(
        "AttributeUpdates.Attr.Value",
        "AttrStruct<FieldStruct>",
        done
      );
    });
  ...
  describe("validations", function () {
    ...
    it("should return ValidationException if expression and non-expression", function (done) {
      assertValidation(
        ...
        "Can not use both expression and non-expression parameters in the same request: " +
          "Non-expression parameters: {AttributeUpdates, Expected} Expression parameters: {UpdateExpression, ConditionExpression}",
        done
      );
    });
      ...
      async.forEach(
        ...
        function (val, cb) {
          assertValidation(
            ...
            "One or more parameter values were invalid: " +
              "DELETE action with value is not supported for the type " +
              Object.keys(val)[0],
            cb
          );
        },
    ...
    it("should return ValidationException if trying to add incorrect types", function (done) {
      async.forEach(
        [{ S: "1" }, { B: "Yg==" }, { NULL: true }, { BOOL: true }, { M: {} }],
        function (val, cb) {
          assertValidation(
            {
              TableName: "abc",
            ...
            "One or more parameter values were invalid: " +
              "ADD action is not supported for the type " +
              Object.keys(val)[0],
            cb
          );
        },
    ...
    it("should return ValidationException if trying to add type B", function (done) {
      assertValidation(
        ...
        "One or more parameter values were invalid: " +
          "ADD action is not supported for the type B",
        done
      );
    });
    ...
    it("should return ValidationException if no value and no exists", function (done) {
      assertValidation(
        { TableName: "abc", Key: {}, Expected: { a: {} } },
        "One or more parameter values were invalid: Value must be provided when Exists is null for Attribute: a",
        done
      );
    });
    ...
    it("should return ValidationException for Exists true with no value", function (done) {
      assertValidation(
        { TableName: "abc", Key: {}, Expected: { a: { Exists: true } } },
        "One or more parameter values were invalid: Value must be provided when Exists is true for Attribute: a",
        done
      );
    });
      ...
      assertValidation(
        {
          TableName: "abc",
          Key: {},
          Expected: { a: { Exists: false, Value: { S: "a" } } },
        },
        "One or more parameter values were invalid: Value cannot be used when Exists is false for Attribute: a",
        done
      );
    });
    ...
    it("should return ValidationException for invalid keys in ExpressionAttributeValues", function (done) {
      assertValidation(
        ...
        'ExpressionAttributeValues contains invalid key: Syntax error; key: "b"',
        done
      );
    });
        ...
        function (expr, cb) {
          assertValidation(
            ...
            "ExpressionAttributeValues contains invalid value: " +
              "Supplied AttributeValue is empty, must contain exactly one of the supported datatypes for key :b",
            cb
          );
        },
        done
    ...
    it("should return ValidationException for multiple datatypes in ExpressionAttributeValues", function (done) {
      assertValidation(
        ...
        "ExpressionAttributeValues contains invalid value: " +
          "Supplied AttributeValue has more than one datatypes set, must contain exactly one of the supported datatypes for key :b",
        done
      );
    });
      ...
      async.forEach(
        ...
        function (expr, cb) {
          assertValidation(
            ...
            'Invalid UpdateExpression: The "' +
              expr[1] +
              '" section can only be used once in an update expression;',
            cb
          );
        },
        done
    ...
    it("should return ValidationException for overlapping paths in UpdateExpression", function (done) {
      async.forEach(
        [
          [
            "set d[1] = a, d.b = a, c[1].a = a, #c = if_not_exists(a)",
            "[c, [1], a]",
            "[c]",
          ],
        ...
        function (expr, cb) {
          assertValidation(
            ...
            "Invalid UpdateExpression: Two document paths overlap with each other; " +
              "must remove or rewrite one of these paths; path one: " +
              expr[1] +
              ", path two: " +
              expr[2],
            cb
    ...
    it("should return ValidationException for conflicting paths in UpdateExpression", function (done) {
      async.forEach(
        [
          [
            "set #c[3].#d = a, #c.#d[3] = if_not_exists(a)",
            "[c, [3], [3]]",
            "[c, [3], [3]]",
          ],
    ...
    it("should return ValidationException for incorrect types in UpdateExpression", function (done) {
      async.forEach(
        [
          [
            "set b = list_append(:a, a), c = if_not_exists(a) add a.b :a",
            { S: "a" },
            "ADD",
            "STRING",
          ],
    ...
    it("should return ValidationException for incorrect number of operands to functions in UpdateExpression", function (done) {
      async.forEach(
        ["set a = if_not_exists(c)", "set a = list_append(c)"],
      ...
      assertValidation(
        {
          ...
          Key: {},
          UpdateExpression:
            "remove a set b = list_append(b, if_not_exists(a, :a))",
    ...
    it("should return ResourceNotFoundException if key is empty and table does not exist", function (done) {
      assertNotFound(
        { TableName: helpers.randomString(), Key: {} },
    ...
    it("should return ValidationException if Key does not match schema", function (done) {
      async.forEach(
        ...
        function (expr, cb) {
          assertValidation(
            { TableName: helpers.testHashTable, Key: expr },
    ...
    it("should return ValidationException if Key does not match range schema", function (done) {
      assertValidation(
        { TableName: helpers.testRangeTable, Key: { a: { S: "a" } } },
        "The provided key element does not match the schema",
        done
      );
    });
    ...
    it("should return ResourceNotFoundException if table is being created", function (done) {
      ...
      request(helpers.opts("CreateTable", table), function (err) {
        ...
        assertNotFound(
          { TableName: table.TableName, Key: { a: { S: "a" } } },
          "Requested resource not found",
          done
        );
    ...
    it("should return ValidationException if trying to update key", function (done) {
      async.forEach(
        [
          ...
          {
            UpdateExpression: "add a.b :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
          {
            UpdateExpression: "delete a :a",
            ExpressionAttributeValues: { ":a": { NS: ["1"] } },
          },
          {
            UpdateExpression: "remove d set b = :a, a = :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
          {
            UpdateExpression: "delete b :a remove a",
            ExpressionAttributeValues: { ":a": { NS: ["1"] } },
          },
        ...
        function (updateOpts, cb) {
          ...
          assertValidation(
            updateOpts,
            "One or more parameter values were invalid: " +
              "Cannot update attribute a. This attribute is part of the key",
            cb
          );
        },
        done
    ...
    it("should return ValidationException if trying to update range key", function (done) {
      async.forEach(
        [
          {
            AttributeUpdates: {
              d: { Value: { N: helpers.randomNumber() } },
              b: { Value: { S: helpers.randomString() } },
            },
          },
          {
            UpdateExpression: "set d[1] = :a add b.b :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
        ],
        function (updateOpts, cb) {
          ...
          assertValidation(
            updateOpts,
            "One or more parameter values were invalid: " +
              "Cannot update attribute b. This attribute is part of the key",
            cb
          );
        },
        done
        ...
        [
          {
            AttributeUpdates: {
              d: { Value: { N: helpers.randomNumber() } },
              c: { Value: { N: helpers.randomNumber() } },
            },
          },
          {
            UpdateExpression: "set d.a = a add c :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
          {
            UpdateExpression: "set e = c[1], c = a + :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
        ],
        function (updateOpts, cb) {
          ...
          assertValidation(
            ...
            new RegExp(
              "^One or more parameter values were invalid: " +
                "Type mismatch for Index Key c Expected: S Actual: N IndexName: index\\d$"
            ),
            cb
          );
        },
    ...
    it("should return ValidationException if trying to update index map", function (done) {
      async.forEach(
        [
          {
            UpdateExpression: "add d.b :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
          {
            UpdateExpression: "set d[1] = :a",
            ExpressionAttributeValues: { ":a": { N: "1" } },
          },
        ...
        function (updateOpts, cb) {
          ...
          assertValidation(
            ...
            "Key attributes must be scalars; " +
              "list random access '[]' and map lookup '.' are not allowed: IndexKey: d",
            cb
          );
        },
        done
        ...
        function (err) {
          ...
          async.forEach(
            [
              {
                AttributeUpdates: {
                  b: { Action: "DELETE", Value: { NS: ["1"] } },
                },
              },
              {
                AttributeUpdates: {
                  b: { Action: "DELETE", Value: { BS: ["YQ=="] } },
                },
              },
              {
                AttributeUpdates: {
                  c: { Action: "DELETE", Value: { NS: ["1"] } },
                },
              },
              {
                AttributeUpdates: {
                  b: { Action: "ADD", Value: { NS: ["1"] } },
                },
              },
      ...
      request(
        ...
        function (err) {
          ...
          async.forEach(
            [
              {
                UpdateExpression: "delete d :a",
                ExpressionAttributeValues: { ":a": { SS: ["1"] } },
              },
              {
                UpdateExpression: "delete b :a",
                ExpressionAttributeValues: { ":a": { NS: ["1"] } },
              },
              {
                UpdateExpression: "delete b.a :a",
                ExpressionAttributeValues: { ":a": { NS: ["1"] } },
              },
              {
                UpdateExpression: "delete b.a :a",
                ExpressionAttributeValues: { ":a": { BS: ["YQ=="] } },
              },
              {
                UpdateExpression: "delete c :a",
                ExpressionAttributeValues: { ":a": { NS: ["1"] } },
              },
              {
                UpdateExpression: "add b :a",
                ExpressionAttributeValues: { ":a": { NS: ["1"] } },
              },
              {
                UpdateExpression: "add d :a",
                ExpressionAttributeValues: { ":a": { N: "1" } },
              },
              {
                UpdateExpression: "set e = a + :a",
                ExpressionAttributeValues: { ":a": { N: "1" } },
              },
              {
                UpdateExpression: "set e = b - :a",
                ExpressionAttributeValues: { ":a": { N: "1" } },
              },
              {
                UpdateExpression:
                  "set e = list_append(d, if_not_exists(f, :a))",
                ExpressionAttributeValues: { ":a": { L: [] } },
              },
            ],
            function (updateOpts, cb) {
              updateOpts.TableName = helpers.testHashTable;
    ...
    it("should return ValidationException if update item is too big", function (done) {
      ...
      var updates = {
        b: {
          Action: "PUT",
          Value: { S: new Array(helpers.MAX_SIZE).join("a") },
        },
        c: {
          Action: "PUT",
          Value: { N: new Array(38 + 1).join("1") + new Array(89).join("0") },
        },
      };
      assertValidation(
        {
  ...
  describe("functionality", function () {
    ...
    it("should just add item with key if no action", function (done) {
      var key = { a: { S: helpers.randomString() } };
      request(
        opts({ TableName: helpers.testHashTable, Key: key }),
        function (err, res) {
          if (err) return done(err);
          res.statusCode.should.equal(200);
          res.body.should.eql({});
    ...
    it("should return updated old values when they exist", function (done) {
      var key = { a: { S: helpers.randomString() } },
        updates = { b: { Value: { S: "a" } }, c: { Value: { S: "a" } } };
      request(
        opts({
          TableName: helpers.testHashTable,
          Key: key,
    ...
    it("should just add valid ADD actions if nothing exists", function (done) {
      async.forEach(
        [
          ...
          {
            UpdateExpression:
              "ADD #e :e,#d :d DELETE #c :c REMOVE #b SET #f = :f",
            ExpressionAttributeValues: {
              ":c": { SS: ["a", "b"] },
              ":d": { N: "5" },
              ":e": { SS: ["a", "b"] },
    ...
    it("should delete normal values and return updated on index table", function (done) {
      var key = {
        ...
        updates = { c: { Value: { S: "a" } }, d: { Value: { S: "a" } } };
      request(
        opts({
          TableName: helpers.testRangeTable,
          Key: key,
    ...
    it("should return ConsumedCapacity for creating small item", function (done) {
      var key = { a: { S: helpers.randomString() } },
        b = new Array(1010 - key.a.S.length).join("b"),
        updates = {
          b: { Value: { S: b } },
          c: { Value: { N: "12.3456" } },
          d: { Value: { B: "AQI=" } },
          e: { Value: { BS: ["AQI=", "Ag==", "AQ=="] } },
        },
        req = {
          TableName: helpers.testHashTable,
          Key: key,
    ...
    it("should return ConsumedCapacity for creating larger item", function (done) {
      var key = { a: { S: helpers.randomString() } },
        b = new Array(1012 - key.a.S.length).join("b"),
        updates = {
          b: { Value: { S: b } },
          c: { Value: { N: "12.3456" } },
          d: { Value: { B: "AQI=" } },
    ...
    it("should return ConsumedCapacity for creating and updating small item", function (done) {
      var key = { a: { S: helpers.randomString() } },
        b = new Array(1009 - key.a.S.length).join("b"),
        updates = {
          b: { Value: { S: b } },
          c: { Value: { N: "12.3456" } },
          d: { Value: { B: "AQI=" } },
    ...
    it("should return ConsumedCapacity for creating and updating larger item", function (done) {
      var key = { a: { S: helpers.randomString() } },
        b = new Array(1011 - key.a.S.length).join("b"),
        updates = {
          b: { Value: { S: b } },
          c: { Value: { N: "12.3456" } },
          d: { Value: { B: "AQI=" } },
    ...
    it("should update when falsey in condition matches", function (done) {
      async.forEach(
        [{ BOOL: false }, { S: "" }, { N: "0.0" }, { N: "0" }, { NULL: true }],
        function (value, cb) {
          var item = {
            a: { S: helpers.randomString() },
            updated: { BOOL: false },
              ...
              request(
                ...
                function (err, res) {
                  ...
                  res.statusCode.should.equal(
                    200,
                    `Update failed when checking for {${
                      Object.keys(value)[0]
                    }:${Object.values(value)[0]}}`
                  );
                  res.body.should.eql({
                    Attributes: { updated: { BOOL: true } },
                  });
                  cb();
                }
              );
        ...
        function (err, res) {
          ...
          request(
            opts({
              ...
              Key: key,
              UpdateExpression:
                "set b.c=((c[1])+(b.a)),b.a = a,c[1] = a, c[4] = b.a - b.b, c[2] = b.c add c[8] :b, c[6] :a",
              ExpressionAttributeValues: {
                ":a": { N: "2" },
                ":b": { SS: ["a"] },
              },
    ...
    it("should update indexed attributes", function (done) {
      ...
      request(
        ...
        function (err, res) {
          ...
          res.statusCode.should.equal(200);
          res.body.should.eql({
            Attributes: { c: key.a, d: key.b, e: key.a, f: key.b },
          });
          request(
            helpers.opts("Query", {
              TableName: helpers.testRangeTable,
