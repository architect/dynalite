...
module.exports = function transactWriteItem (store, data, cb) {
  ...
  function addActions (transactItem, cb) {
    ...
    if (transactItem.Put) {
      ...
      store.getTable(tableName, function (err, table) {
        if (err) return cb(err)
        let invalid = db.validateItem(transactItem.Put.Item, table)
        ...
        itemDb.get(key, function (err, oldItem) {
          ...
          let invalid = db.checkConditional(transactItem.Put, oldItem)
          if (invalid != null) return cb(invalid)
      ...
      store.getTable(tableName, function (err, table) {
        if (err) return cb(err)
        let invalid =  db.validateKey(transactItem.Delete.Key, table)
        if (invalid != null) return cb(invalid)
        ...
        itemDb.lock(key, function (release) {
          ...
          itemDb.get(key, function (err, oldItem) {
            ...
            let invalid = db.checkConditional(transactItem.Delete, oldItem)
            if (invalid != null) return cb(invalid)
      ...
      store.getTable(tableName, function (err, table) {
        ...
        let invalid = db.validateUpdates(transactItem.Update.AttributeUpdates, transactItem.Update._updates, table)
        if (invalid != null) return cb(invalid)
        ...
        itemDb.lock(key, function (release) {
          ...
          itemDb.get(key, function (err, oldItem) {
            ...
            let invalid = db.checkConditional(transactItem.Update, oldItem)
            if (invalid != null) return cb(invalid)
      ...
      store.getTable(tableName, function (err, table) {
        ...
        itemDb.lock(key, function (release) {
          ...
          itemDb.get(key, function (err, oldItem) {
            ...
            let invalid = db.checkConditional(transactItem.ConditionCheck, oldItem)
            if (invalid != null) return cb(invalid)
