...
module.exports = function updateTable(store, data, cb) {
  ...
  tableDb.lock(key, function (release) {
    ...
    store.getTable(key, false, function (err, table) {
      ...
      var tableBillingMode =
        (table.BillingModeSummary || {}).BillingMode || "PROVISIONED";

      if (
        data.ProvisionedThroughput &&
        (data.BillingMode || tableBillingMode) == "PAY_PER_REQUEST"
      ) {
        return cb(
          db.validationError(
      ...
      for (i = 0; i < updates.length; i++) {
        ...
        readDiff =
          dataThroughput.ReadCapacityUnits - tableThroughput.ReadCapacityUnits;
        writeDiff =
          dataThroughput.WriteCapacityUnits -
          tableThroughput.WriteCapacityUnits;
          ...
          return cb(
            db.validationError(
              "The provisioned throughput for the table will not change. The requested value equals the current value. " +
                ...
                tableThroughput.ReadCapacityUnits +
                ". Requested ReadCapacityUnits: " +
                dataThroughput.ReadCapacityUnits +
                ". " +
      ...
      tableDb.put(key, table, function (err) {
        ...
        setTimeout(function () {
          ...
          if (
            ...
            table.BillingModeSummary.LastUpdateToPayPerRequestDateTime =
              Date.now() / 1000;
            table.TableThroughputModeSummary.LastUpdateToPayPerRequestDateTime =
              Date.now() / 1000;
...
function getThroughputUpdates(data, table) {
  var tableBillingMode =
    (table.BillingModeSummary || {}).BillingMode || "PROVISIONED";
  var remainingIndexes = (table.GlobalSecondaryIndexes || []).reduce(function (
    map,
    index
  ) {
  ...
  globalUpdates.forEach(function (update) {
    ...
    if (
      dataThroughput.ReadCapacityUnits > 1000000000000 ||
      dataThroughput.WriteCapacityUnits > 1000000000000
    ) {
  ...
  if (
    data.BillingMode == "PROVISIONED" &&
    tableBillingMode != "PROVISIONED" &&
    Object.keys(remainingIndexes).length
  ) {
    throw db.validationError(
      "One or more parameter values were invalid: " +
        "ProvisionedThroughput must be specified for index: " +
        Object.keys(remainingIndexes).join(",")
    );
  }
  return updates;
}
