...
module.exports = function batchWriteItem(store, data, cb) {
  ...
  function addTableActions(tableName, cb) {
    store.getTable(tableName, function (err, table) {
      ...
      var reqs = data.RequestItems[tableName],
        i,
        req,
        key,
        seenKeys = {},
      ...
      for (i = 0; i < reqs.length; i++) {
        ...
        if (req.PutRequest) {
          if ((err = db.validateItem(req.PutRequest.Item, table)) != null)
            return cb(err);
          ...
          if ((err = db.validateKey(req.DeleteRequest.Key, table) != null))
            return cb(err);
        ...
        if (seenKeys[key])
          return cb(
            db.validationError("Provided list of item keys contains duplicates")
          );
        seenKeys[key] = true;
      }
