  ...
  store.getTable(data.TableName, function (err, table) {
    ...
    if (
      ...
      return data.IndexName
        ? cb(db.validationError("The provided starting key is invalid"))
        : cb(
            db.validationError(
    ...
    if (data.TotalSegments > 1) {
      if (data.Segment > 0)
        var hashStart = (
          "00" +
          Math.ceil((4096 * data.Segment) / data.TotalSegments).toString(16)
        ).slice(-3);
      var hashEnd =
        (
          "00" +
          (
            Math.ceil((4096 * (data.Segment + 1)) / data.TotalSegments) - 1
          ).toString(16)
        ).slice(-3) + "~";
    }

    if (data.ExclusiveStartKey) {
      var startKey = db.createIndexKey(
        data.ExclusiveStartKey,
        table,
        keySchema
      );
      ...
      if (
        ...
        (startKey < hashStart || startKey > hashEnd)
      ) {
        return cb(
          db.validationError(
