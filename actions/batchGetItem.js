...
module.exports = function batchGetItem(store, data, cb) {
  ...
  async.series(
    ...
    function (err, responses) {
      ...
      var res = { Responses: {}, UnprocessedKeys: {} },
        table,
        tableResponses = responses[1],
        totalSize = 0,
        capacities = {};
      ...
      for (table in tableResponses) {
        ...
        tableResponses[table].forEach(function (tableRes, ix) {
          tableRes._key = data.RequestItems[table].Keys[ix];
        }); // eslint-disable-line no-loop-func
        shuffle(tableResponses[table]);
        res.Responses[table] = tableResponses[table]
          .map(function (tableRes) {
            // eslint-disable-line no-loop-func
            if (tableRes.Item) {
              ...
              if (newSize > 1024 * 1024 + store.options.maxItemSize - 3) {
                if (!res.UnprocessedKeys[table]) {
                  ...
                  if (data.RequestItems[table].AttributesToGet)
                    res.UnprocessedKeys[table].AttributesToGet =
                      data.RequestItems[table].AttributesToGet;
                  if (data.RequestItems[table].ConsistentRead)
                    res.UnprocessedKeys[table].ConsistentRead =
                      data.RequestItems[table].ConsistentRead;
                }
      ...
      if (~["TOTAL", "INDEXES"].indexOf(data.ReturnConsumedCapacity)) {
        res.ConsumedCapacity = Object.keys(tableResponses).map(function (
          table
        ) {
          return {
  ...
  function addTableRequests(tableName, cb) {
    store.getTable(tableName, function (err, table) {
      ...
      var req = data.RequestItems[tableName],
        i,
        key,
        options,
        gets = [];
      ...
      requests[tableName] = async.map.bind(async, gets, function (data, cb) {
        return getItem(store, data, cb);
      });
