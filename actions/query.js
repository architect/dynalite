...
module.exports = function query(store, data, cb) {
  ...
  store.getTable(data.TableName, function (err, table) {
    ...
    var keySchema = table.KeySchema,
      startKeyNames = keySchema.map(function (key) {
        return key.AttributeName;
      }),
      hashKey = startKeyNames[0],
      rangeKey = startKeyNames[1],
      fetchFromItemDb = false,
      isLocal;

    if (data.IndexName) {
      var index = db.traverseIndexes(
        table,
        function (attr, type, index, isGlobal) {
      ...
      if (index == null) {
        return cb(
          db.validationError(
            "The table does not have the specified index: " + data.IndexName
          )
        );
      }
      ...
      fetchFromItemDb =
        data.Select == "ALL_ATTRIBUTES" &&
        index.Projection.ProjectionType != "ALL";
      keySchema.forEach(function (key) {
        if (!~startKeyNames.indexOf(key.AttributeName))
          startKeyNames.push(key.AttributeName);
      });
    ...
    if (
      ...
      Object.keys(data.ExclusiveStartKey).length != startKeyNames.length
    ) {
    ...
    err = db.traverseKey(table, keySchema, function (attr, type, isHash) {
      ...
      if (
        isHash &&
        keySchema.length == 1 &&
        Object.keys(data.KeyConditions).length > 1
      ) {
      ...
      if (!data.KeyConditions[attr]) {
        if (isHash || Object.keys(data.KeyConditions).length > 1) {
          return db.validationError(
            "Query condition missed key schema element: " + attr
          );
        }
        return;
      }
      ...
      if (
        ~["NULL", "NOT_NULL", "NE", "CONTAINS", "NOT_CONTAINS", "IN"].indexOf(
          comparisonOperator
        )
      ) {
        return db.validationError(
      ...
      if (
        data.KeyConditions[attr].AttributeValueList.some(function (attrVal) {
          return attrVal[type] == null;
        })
      ) {
        return db.validationError(
    ...
    var hashType = Object.keys(
      data.KeyConditions[hashKey].AttributeValueList[0]
    )[0];
    ...
    if (data.ExclusiveStartKey) {
      ...
      if ((err = db.validateKey(tableStartKey, table)) != null) {
        return cb(
          db.validationError(
            "The provided starting key is invalid: " + err.message
          )
        );
      }
      ...
      if (!rangeKey || !data.KeyConditions[rangeKey]) {
        if (data.ExclusiveStartKey[hashKey][hashType] != hashVal) {
          return cb(
            db.validationError(
              "The provided starting key is outside query boundaries based on provided conditions"
            )
          );
        }
      } else {
        var matchesRange = db.compare(
          data.KeyConditions[rangeKey].ComparisonOperator,
          data.ExclusiveStartKey[rangeKey],
          data.KeyConditions[rangeKey].AttributeValueList
        );
        if (!matchesRange) {
          return cb(
            db.validationError(
    ...
    if (
      (err = db.validateKeyPaths(
        (data._projection || {}).nestedPaths,
        table
      )) != null
    )
      return cb(err);
    ...
    if (
      (err = db.validateKeyPaths((data._filter || {}).nestedPaths, table)) !=
      null
    )
      return cb(err);

    var opts = {
      reverse: data.ScanIndexForward === false,
      limit: data.Limit ? data.Limit + 1 : -1,
    };

    opts.gte =
      db.hashPrefix(hashVal, hashType) +
      "/" +
      db.toRangeStr(hashVal, hashType) +
      "/";
    opts.lt = opts.gte + "~";

    if (data.KeyConditions[rangeKey]) {
      var rangeStrPrefix = db.toRangeStr(
        data.KeyConditions[rangeKey].AttributeValueList[0]
      );
      ...
      if (comp == "EQ") {
        ...
        opts.lte =
          opts.gte +
          db.toRangeStr(data.KeyConditions[rangeKey].AttributeValueList[1]) +
          "/~";
        opts.gte += rangeStr;
        delete opts.lt;
      }
