...
const target = 'PutItem'
// Bind helper functions
// const request = helpers.request; // Marked as unused by linter
// const randomName = helpers.randomName; // Marked as unused by linter
...
test('putItem', (t) => {
  t.test('serializations', (st) => {
    ...
    st.test('should return SerializationException when Item is not a map', (sst) => {
      assertType('Item', 'Map<AttributeValue>', (err) => {
        sst.error(err, 'assertType should not error')
        sst.end()
      })
    })

    st.test('should return SerializationException when Item.Attr is not an attr struct', (sst) => {
      // Timeout removed
      assertType('Item.Attr', 'AttrStruct<ValueStruct>', (err) => {
        sst.error(err, 'assertType should not error')
        sst.end()
      })
    })
  ...
  // The 'validations' and 'functionality' blocks seem to be in putItem.part2.js etc.
  t.end() // End putItem tests
})
